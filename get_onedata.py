...
def format_database_data(pages):
    ...
    for page in pages:
        ...
        entry_file_path = os.path.join(output_dir, f"entry{entry_counter}.txt")
        with open(entry_file_path, "w", encoding="utf-8") as entry_file:
                # Combine the entry into a single string
            entry_text = "\n".join(entry)
            # Write entry and block_content to the file with a newline separator
            entry_file.write(entry_text)
            entry_file.write("\n")  # Add a newline between entry and block_content
            entry_file.write(block_content)
        entry_counter += 1
...
def parse_llm_response(response):
    ...
    if descriptions_match:
        ...
        descriptions = {key: desc for key, desc in (item.split(": ") for item in descriptions_str.split("\n"))}
    ...
    for entry in content_list:
        ...
        if current_chunk_length + entry_length > MAX_CHARACTERS:
            ...
            if current_chunk:
                ...
                content_match = extract_cleaned_content(cleaned_content)
                content_elements = [item.strip() for item in content_match.split("\n\n") if item.strip()]
...
def create_prompt_for_chunk(content_chunk, descriptions):
    ...
    prompt = (
        "### Instructions\n"
        "The structure of the content should remain fixed—only the values of the keys should be translated into English and refined according to the provided key descriptions.\n"
        "Do not modify the order or number of entries. The entries should remain exactly the same as in the input.\n"
        "Ensure the cleaned and translated content starts strictly with 'Content: ...'.\n\n"
        f"<Key Descriptions>\n{descriptions}\n\n"
        f"<Content>\n{'\n\n'.join(content_chunk)}\n\n"
    )
    return prompt
...
def extract_cleaned_content(response):
    ...
    """
    # Ensure the response contains the cleaned content in the expected format
    content_match = re.search(r"Content:\s*(.*)", response, re.S)
    if content_match:
        return content_match.group(1).strip()
    else:
        raise ValueError("The LLM response did not return content in the expected format.")
...
TARGET_DATABASE_ID = "5919f511-5959-4a28-b5f3-c415b9f0e4fd"
TARGET_DATABASE_NAME = "Blog post"
# Function to fetch databases
def fetch_databases():
    print(f"\nProcessing database ID: {TARGET_DATABASE_ID}, Name: {TARGET_DATABASE_NAME}")
    process_database(TARGET_DATABASE_ID, TARGET_DATABASE_NAME)
